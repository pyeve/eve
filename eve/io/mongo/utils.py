...
class GeoJSON(dict):
    ...
    def _correct_position(self,position):
        return isinstance(position,list) and isinstance(position[0],float) and isinstance(position[1],float)
...
class Geometry(GeoJSON):
    ...
    def __init__(self,json):
        ...
        try:
            if not isinstance(self['coordinates'],list) or self['type'] != self.__class__.__name__:
                raise TypeError
